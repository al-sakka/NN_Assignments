...
The baseline compresses each utterance into a \textbf{180-D summary vector}:
\[
    \mathbf{x}_{\text{summary}} = \big[\,\mu(\text{MFCC}),\;\sigma(\text{MFCC}),\;
    \mu(\Delta),\;\sigma(\Delta),\;
    \mu(\Delta^2),\;\sigma(\Delta^2)\,\big]
    \;\in\;\mathbb{R}^{180}
\]
where $\mu$ and $\sigma$ denote the mean and standard deviation across all frames.
This discards temporal ordering but retains global spectral statistics.

An \textbf{SVM} with RBF kernel is trained via \texttt{GridSearchCV} (5-fold CV) over
$C \in \{1, 10, 100, 1000\}$ and $\gamma \in \{\text{scale}, 0.01, 0.001\}$.
...
\begin{itemize}
    \item \textbf{Encoder:} $2430 \to 1024 \to 512 \to 256$ (each with BatchNorm + ReLU)
...
The classification loss $\mathcal{L}_{\text{cls}}$ is the cross-entropy loss of the
classifier head attached to the latent layer; it drops to $\approx 0$ by epoch~120,
indicating perfect classification on training data.
The reconstruction loss continues to decrease, reaching $\approx 0.073$ by epoch~500.
...
AE Classifier Head              & 86.00  & \multirow{3}{*}{2244.1} & \multirow{3}{*}{92.9}    \\
AE Latent + SVM                 & 89.33  &       &     \\
\textbf{AE Hybrid + SVM (best)} & \textbf{90.67} &  &     \\
\bottomrule
\end{tabular}
...
\midrule
MFCCs                & 30 coefficients \\
Frame / Hop          & 15\,ms / 10\,ms \\
Latent dimension     & 256 \\
AE epochs            & 500 \\
Augmentation         & 3 noisy copies ($\sigma=0.05$) \\
Loss weight ($\lambda$) & 2.0 \\
SVM kernel           & RBF \\
Best SVM params      & $C=1$, $\gamma=\text{scale}$ \\
\bottomrule
\end{tabular}
\caption{Problem~4: Key hyperparameters.}
\label{tab:prob4_params}
\end{table}
...
(each independently standardised).  This works because:
\begin{itemize}
    \item The latent vector captures temporal patterns the summary misses.
    \item The summary features capture global statistics (mean/std of deltas)
          that complement the AE's learned representation.
    \item Separate standardisation prevents one feature set from dominating due
          to scale differences.
\end{itemize}

\subsubsection{Supervised AE Joint Loss}

Adding the classification loss ($\lambda = 2.0$) to the reconstruction loss
\textbf{shapes the latent space to be class-discriminative}, not just
reconstruction-optimal.  The classification loss drops to zero on training
data by epoch~120, while reconstruction loss continues to decrease ---
the model simultaneously learns to reconstruct and classify.
